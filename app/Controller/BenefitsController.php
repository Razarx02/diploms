 ...
 */
    // public $uses = array('Adventage', "Benefit");
	...
	public function admin_add() {
        if( $this->request->is('post') ){
			...
			if( $this->Benefit->save($data) ){
				$this->Session->setFlash('Преимущества добавлена', 'default', array(), 'good');
    ...
    public function admin_edit($id)
    {
        ...
        if( is_null($id) || !(int)$id || !$this->Benefit->exists($id)){
			throw new NotFoundException('Такой страницы нету...');
		}

		$data = $this->Benefit->findById($id);
		...
		if($this->request->is(array('post', 'put'))){
            $this->Benefit->id = $id;  //  Указываем id поста
			$data1 = $this->request->data["Benefit"];
			...
			if($this->Benefit->save($data1)){
				$this->Session->setFlash('Изменение прошло успешно', 'default', array(), 'good');
				if (!empty($data1["img"]["name"])) {
					if($data1["img"]["name"] != $data["Benefit"]["img"] ) {
						unlink(WWW_ROOT . 'img\benefits\\' . $data["Benefit"]["img"]);  // Удалить картинку с катлога img 
						unlink(WWW_ROOT . 'img\benefits\thumbs\\' . $data["Benefit"]["img"]); // Удалить картинку с катлога img 
					}
				}
				return $this->redirect(array("action" => "index"));
			} else {
