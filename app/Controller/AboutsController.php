 ...
 */
	public $uses = array( "About","About", "Benefit", "Brand");
	...
	{	
		...
		$this->set(compact( "about", "benefits", "brands"));
	}
    ...
    public function admin_edit($id)
    {
		...
		if($this->request->is(array('post', 'put'))){
            $this->About->id = $id;  //  Указываем id поста
			...
			if($this->About->save($data1)){
				...
				if (!empty($data1["img"]["name"])) {
					if($data1["img"]["name"] != $data["About"]["img"] ) {
						unlink(WWW_ROOT . 'img\abouts\\' . $data["About"]["img"]);  // Удалить картинку с катлога img 
						unlink(WWW_ROOT . 'img\abouts\thumbs\\' . $data["About"]["img"]); // Удалить картинку с катлога img 
					}
				}
				return $this->redirect(array("action" => "index"));
			} else {
